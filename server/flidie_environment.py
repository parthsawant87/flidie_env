...
from server.graders import grade_easy, grade_medium, grade_hard, _to_open_unit
from server.tasks   import get_task, list_tasks, TaskConfig
...
class FlidieEnv:
    ...
    def step(self, action: FinancialAction) -> StepResult:
        ...
        if done:
          ...
          reward_clamped = max(0.01, min(0.99, _to_open_unit(reward)))
        ...
        return StepResult(
            observation = obs,
            reward      = round(max(0.01, min(0.99, reward_clamped)), 4),
            done        = done,
            info        = {
