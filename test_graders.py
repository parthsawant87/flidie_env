...
from graders import grade_easy, grade_medium, grade_hard, TIER_REWARD, HARD_CALC_GATE
...
HARD_GT = {
    ...
    "key_calculations": ["10000 * (200 - 10)", "1900000 * 0.3"],  # = 1900000, 570000
...
def test_easy_perfect_score():
    ...
    assert score == 1.00, f"Optimal choice should score 1.00, got {score}"

def test_easy_illegal_choice():
    """Illegal choice must produce -1.00 base (before any calculation bonus)."""
    score = grade_easy([choose("D")], EASY_GT)
    assert score == -1.00, f"Illegal choice must score -1.00, got {score}"

def test_easy_no_action_returns_zero():
    score = grade_easy([ask(), ask()], EASY_GT)
    assert score == 0.0, f"No choose_option should return 0.0, got {score}"
...
def test_easy_first_choice_only():
    ...
    assert score == TIER_REWARD["good"], \
        f"Only first choice counts. Should be {TIER_REWARD['good']}, got {score}"

def test_easy_calculation_bonus():
    """Correct calculate() call earns bonus on top of base score."""
    base  = grade_easy([choose("B")], EASY_GT)               # no calc
    ...
    assert bonus - base > 0.04, f"Bonus should be approximately +0.05, got {bonus - base}"
...
def test_medium_illegal_step_tanks_score():
    """Step 2 illegal choice tanks the weighted average."""
    actions = [choose("A"), choose("B"), choose("B")]  # step 2 B = illegal
    score = grade_medium(actions, MEDIUM_GT)
    assert score < 0.50, f"Illegal step 2 should drop score below 0.50, got {score}"

def test_medium_calculation_bonus():
    """Two correct calculations earn up to +0.20 bonus for medium."""
...
def test_hard_perfect_score_all_dimensions():
    """Optimal choice + correct trap + correct escalation + correct calc."""
    actions = [
...
def test_hard_illegal_choice_no_calc_bonus():
    """
    Illegal choice → decision_raw = -1.0 → gate fails → no calculation bonus.
    Even if agent called calculate() perfectly, no calc bonus applies.
    """
...
def test_hard_false_alarm_penalty():
    """Flag compliance when no trap exists → -0.40 on trap dimension."""
...
def test_hard_wrong_trap_section_no_credit():
    """Flag the wrong law section → 0.0 on trap (no bonus, no penalty)."""
...
def test_hard_unnecessary_escalation_penalised():
    """Escalate when needs_escalation=False → negative escalation component."""
...
def test_hard_empty_history_returns_zero():
    ...
    assert score >= -0.01 and score <= 0.01, \
        f"Empty history should return ~0.0, got {score}"
...
def test_all_graders_are_deterministic():
    """
    Run each grader 100 times with identical inputs.
    All scores must be identical. If any varies, the grader is non-deterministic
    and the entire environment is disqualified.
    """
